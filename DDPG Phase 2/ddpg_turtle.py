...
class Agent(object):
    ...
    def learn(self):
        ...
        # Finding the actions, and Q-values using the target network
        ...
        for j in range(self.batch_size):
            # Predicted target value using target network
        ...
        # Making the gradients back to zero to avoid accumulation
        critic_loss = F.mse_loss(target, critic_value)
        # critic_loss is the mean square error between target and calculated critic_value 
        # using the actor and critic regular networks
        critic_loss.backward()
        # Computing the gradients of critic_loss wrt critic network parameters
        self.critic.optimizer.step()
        # Updates Model parameters based on computed gradients above

        self.critic.eval()  # evaluation mode (to calculate the loss for Actor network)
        self.actor.optimizer.zero_grad()
        # Getting the action for the state from actor network
        mu = self.actor.forward(state)
        self.actor.train()
        # Gradient ascent getting Q-value of action using critic
        actor_loss = -self.critic.forward(state, mu)
        # Finding the mean loss
        actor_loss = T.mean(actor_loss)
        # Computing the gradients wrt network parametes
        actor_loss.backward()
...
def ddpg(n_games):
    ...
    for i in range(n_games):
        ...
        if i % 25 == 0:
            agent.save_models()
        filename = 'Turtle_' + str(n_games) + '_games.png'
    ...
    plotter(dist_travelled, filename='Episode_ddpg_train_dist.png', ylab = 'Units', window = 100)
    plotter(eps_training_times, filename='Episode_ddpg_train_time_curve.png',ylab = 'Seconds', window = 100)
    plotLearning(score_history, filename, window=100)
    agent.save_models()
    T.save(agent.actor.state_dict(), 'tmp/ddpg/Actor_ddpg.pth')
    T.save(agent.critic.state_dict(), 'tmp/ddpg/Critic_ddpg.pth')
    T.save(agent.target_actor.state_dict(), 'tmp/ddpg/TargetActor_ddpg.pth')
    T.save(agent.target_critic.state_dict(), 'tmp/ddpg/TargetCritic_ddpg.pth')
